...
//TODO attacker + 1 solution


/** 
 * @notice Solution1: limit the max proposal
 * @notice Solution2: progressive tally, on each vote
 * @notice Solution3: a paginating system to tally the table with multiple calls .. more complex 
 */
contract Voting is Ownable {
    ...
    // TODO: check or use my own implem 
    function vote(uint proposalId) public flowStatus(WorkflowStatus.VotingSessionStarted) {
        require(voters[msg.sender].isRegistered, "You are not allowed to vote");
        require(!voters[msg.sender].hasVoted, "You have already voted");
        proposals[proposalId].voteCount += 1;
