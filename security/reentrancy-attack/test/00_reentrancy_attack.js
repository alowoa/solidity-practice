...
describe("Reentrency vault attack", function () {

  describe("Vault - Unprotected Vault", function () {
    ...
    before(async () => {
      ...
      vaultContract = await ethers.deployContract("Vault"); 
    });
    ...
    describe("Attack scenario", function () {
      ...
      it("attacker start the attach with the 1st redeem", async function () {
        // WHEN
        await attackerContract.connect(attackerAccount).redeem();
        // THEN
        expect(await attackerContract.connect(attackerAccount).getAttackerBalance()).greaterThan(ATTACKER_STORE_AMOUNT);
      });
  ...
  describe("VaultV2 - contract balance update first protected vault", function () {
    ...
    describe("Attack scenario with VaultV2 - update balance first as protection", function () {
      ...
      it("attacker start the attach should failed to get more than his initial stored amount", async function () {
        ...
        await attackerContract.connect(attackerAccount).redeem();
        // THEN
        expect(await attackerContract.connect(attackerAccount).getAttackerBalance()).lessThanOrEqual(ATTACKER_STORE_AMOUNT);
      });
  
    }); //! VaultV2 - contract balance update first protected vault
